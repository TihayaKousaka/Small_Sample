...
GPU='0 1 2 3'
TEST_BATCH=128
TRAIN_FILE=/root/autodl-tmp/CCF-BDCI-fewshot-classification-main/data/fewshot/train_632_aug_tail.json
LABEL=36
DROPOUT=0.3
MODEL=model1
# 直接联网下载模型文件
...
python main.py \
--train \
--batch 128 --board --datetime ${TIMESTAMP} --epoch 40 --gpu ${GPU} --lr 2e-5 --seed ${SEED} --early_stop 10 \
...
--data_folder_dir fewshot --data_file ${TRAIN_FILE} --label ${LABEL} \
--bert ${BERT} --dropout ${DROPOUT} --feature_layer 4 --freeze 0 --model ${MODEL} --seed ${SEED} \
--K ${K}
# --en \
